...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('weekly_extractor_v3.log', encoding='utf-8')
    ]
)
...
class NeoAutoWeeklyScraper:
    def __init__(self):
        ...
        # Base URL templates for brand search (ahora incluye seminuevos)
        self.BASE_URL_TEMPLATES = [
            "https://neoauto.com/venta-de-autos-usados-{brand}",
            "https://neoauto.com/venta-de-autos-seminuevos-{brand}"
        ]
        self.HEADERS = {
    ...
    def get_total_pages(self, url: str) -> int:
        try:
            ...
            # Intento 3: JSON escapado (Nuevo formato Neoauto)
            if total_items == 0:
                 total_match_esc = re.search(r'\\"total\\":\s*(\d+)', response.text)
                 if total_match_esc:
    ...
    def scrape_week(self):
        ...
        for brand in self.brand_mapping.keys():
            ...
            for base_url_template in self.BASE_URL_TEMPLATES:
                base_url = base_url_template.format(brand=brand)
                logging.info(f"-> Explorando Categoría: {base_url}")
                ...
                for page in range(1, total_pages + 1):
                    ...
                    if not page_data:
                        logging.info(f"   Página {page}: 0 autos. Siguiente categoría/marca...")
                        break
