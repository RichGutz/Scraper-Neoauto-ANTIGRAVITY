...
def advanced_scraping(url: str, page):
    ...
    try:
        ...
        try:
            data['precio'] = page.evaluate(r'''() => {
                const priceElem = document.querySelector('.text-title-large, .text-title-medium');
                return priceElem ? parseFloat(priceElem.innerText.replace(/[^\d.]/g, '')) : null;
            }''')
            if data['precio']:
        ...
        try:
            data['kilometraje'] = page.evaluate(r'''() => {
                const kmLabel = Array.from(document.querySelectorAll('div, span')).find(el => 
                    el.textContent.trim().toLowerCase() === 'kilometraje'
                );
                return kmLabel && kmLabel.nextElementSibling ? parseInt(kmLabel.nextElementSibling.textContent.replace(/[^\d]/g, '')) : null;
            }''')
            if data['kilometraje']:
        ...
        try:
            data['ubicacion'] = page.evaluate(r'''() => {
                const titleMatch = document.title.match(/ en ([A-Za-z\s]+),/);
                if (titleMatch) return titleMatch[1].trim();
                
                const locLabel = Array.from(document.querySelectorAll('span')).find(el => 
                    el.textContent.trim() === 'Lima' || el.textContent.trim() === 'Callao'
                );
                return locLabel ? locLabel.textContent.trim() : null;
            }''')
        ...
        try:
            data['transmision'] = page.evaluate(r'''() => {
                const transmissionLabel = Array.from(document.querySelectorAll('div, span')).find(el => 
                    el.textContent.trim().toLowerCase() === 'transmisión' || el.textContent.trim().toLowerCase() === 'transmision'
                );
                return transmissionLabel && transmissionLabel.nextElementSibling ? transmissionLabel.nextElementSibling.textContent.trim() : null;
            }''')
            if data['transmision']:
        ...
        try:
            data['descripcion'] = page.evaluate(r'''() => {
                const descLabel = Array.from(document.querySelectorAll('h2, h3, span, div')).find(el => 
                    el.textContent.trim().toLowerCase() === 'descripción'
                );
                return descLabel && descLabel.nextElementSibling ? descLabel.nextElementSibling.innerText.trim() : null;
            }''')
            if data['descripcion']:
