...
def create_or_get_folder(service, name, parent_id=None):
    try:
        ...
        print(f"Ocurrió un error al crear/obtener la carpeta: {error}", file=sys.stderr)
        return None
...
def upload_file(service, file_path, folder_id):
    try:
        ...
        if existing_files: # Actualizar archivo existente
            ...
            print(f"Archivo actualizado: {file_path.name}", file=sys.stderr)
        else: # Subir nuevo archivo
            service.files().create(body=file_metadata, media_body=media, fields='id').execute()
            print(f"Archivo subido: {file_path.name}", file=sys.stderr)
        return True
    except HttpError as error:
        print(f"Ocurrió un error al subir el archivo {file_path.name}: {error}", file=sys.stderr)
        return False

def upload_folder_recursive(service, local_folder_path, parent_drive_id):
    print(f"Subiendo carpeta: {local_folder_path.name} a Drive...", file=sys.stderr)
...
def get_shareable_link(service, file_id):
    try:
        ...
        if not has_public_permission:
            ...
            print(f"Permisos de lectura pública añadidos para el archivo {file_id}", file=sys.stderr)

        # Obtener el enlace compartible
        file = service.files().get(fileId=file_id, fields='webViewLink').execute()
        return file.get('webViewLink')
    except HttpError as error:
        print(f"Ocurrió un error al obtener el enlace compartible para {file_id}: {error}", file=sys.stderr)
        return None
...
def main_upload_logic():
    try:
        ...
        if not root_drive_folder_id: 
            print("No se pudo crear/obtener la carpeta raíz en Google Drive.", file=sys.stderr)
            return None
        ...
        for local_folder in LOCAL_FOLDERS_TO_UPLOAD:
            if local_folder.exists():
                ...
                print(f"Advertencia: La carpeta local {local_folder} no existe y no será subida.", file=sys.stderr)
        ...
        if index_html_path.exists():
            ...
            if outputs_drive_folder_id:
                ...
                if files:
                    ...
                    print("index.semanal.html no encontrado en Google Drive después de la subida.", file=sys.stderr)
            else:
                print("No se pudo encontrar la carpeta 'outputs' en Google Drive.", file=sys.stderr)
        else:
            print(f"Advertencia: {index_html_path} no existe localmente. No se generará enlace.", file=sys.stderr)
        
        return None

    except HttpError as error:
        print(f"Ocurrió un error de HTTP en el proceso de subida a Drive: {error}", file=sys.stderr)
        return None
    except Exception as e:
        print(f"Ocurrió un error inesperado en el proceso de subida a Drive: {e}", file=sys.stderr)
        return None

if __name__ == '__main__':
    print("Iniciando drive_uploader.py directamente...", file=sys.stderr)
    link = main_upload_logic()
    if link:
        print(link)
    else:
        print("Proceso de subida a Drive fallido o enlace no generado.", file=sys.stderr)