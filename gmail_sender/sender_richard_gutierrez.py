...
RECIPIENT_EMAILS_FILE = Path(__file__).parent / 'destinatarios.txt' # Use Path to construct absolute path
LOG_FILE = Path(__file__).parent / 'envio_reporte.log' # Use Path to construct absolute path
    ...
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            ...
            if not credentials_path.exists():
                logger.error("CRITICAL ERROR: 'credentials.json' not found at expected path.") # Modified error message
                return None
            flow = InstalledAppFlow.from_client_secrets_file(str(credentials_path), SCOPES)
            creds = flow.run_local_server(port=0)
        with open(token_path, 'w') as token: # Use token_path directly
...
def crear_mensaje_email(destinatario_email, asunto, cuerpo_html, drive_link=None, pdf_path=None): # MODIFIED SIGNATURE
    ...
    if pdf_path:
        try:
            ...
            mensaje.attach(part)
            logger.info(f"Successfully attached PDF: {pdf_path}")
        except Exception as e:
            logger.error(f"Could not attach PDF file {pdf_path}: {e}")
...
def main():
    ...
    logger.info("[Step 3/4] Finding the report file...")
    try:
        script_dir = Path(__file__).parent
        html_file_path = script_dir / "cuerpo_correo_richard_gutierrez.html"
    ...
    RECIPIENT_EMAILS_FILE = Path(__file__).parent / 'destinatarios_richard_gutierrez.txt'
    ...
    new_subject = "Pickup para viaje a Oxapampa!"
