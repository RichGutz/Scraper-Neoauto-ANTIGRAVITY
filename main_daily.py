...
def launch_vpn_and_wait():
    """Lanza la aplicación GUI de ProtonVPN y espera para que el usuario conecte manualmente."""
    vpn_app_path = "/usr/bin/protonvpn-app"
    logger.info(f"Lanzando la aplicación Proton VPN GUI desde: {vpn_app_path}")
    try:
        # Usamos Popen para no esperar a que la GUI se cierre
        subprocess.Popen([vpn_app_path])
        wait_time = 30
        logger.info(f"Esperando {wait_time} segundos para que el usuario conecte la VPN manualmente...")
        logger.info("Por favor, conecta la VPN manualmente en la aplicación GUI de ProtonVPN.")
        time.sleep(wait_time)
        logger.info("Pausa finalizada. Se asume que la VPN está conectada manualmente.")
        return True
    except FileNotFoundError:
        logger.error(f"Error Crítico: No se encontró el ejecutable de Proton VPN GUI en la ruta: {vpn_app_path}")
        return False
    except Exception as e:
        logger.error(f"Ocurrió un error inesperado al lanzar la aplicación VPN: {e}")
        return False
...
def run_script(script_path: Path, script_name: str, script_args: list = []):
    ...
    try:
        ...
        if script_name == "4.DIARIO.SEMANAL.SCRAPER.NEOAUTO.SUPABASE.PARA.CRON.py":
            logger.info(f"Mostrando salida de {script_name} directamente en la terminal (streaming)...")
            ...
            if process.returncode != 0:
                logger.error(f"--- ERROR en {script_name}: El script falló con código de error {process.returncode}. ---")
                return False
        ...
        # If output was captured, log it. If not, it's already printed.
        if e.stdout:
...
def main():
    ...
    scripts_to_run_before_upload = [
        ...
        (base_path / "generador_reporte_mejorado.py", "generador_reporte_mejorado.py", []),
    ]
