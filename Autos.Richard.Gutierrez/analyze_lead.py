...
# --- CONFIGURATION & SETUP ---
# Load both .env files for dual database access
# Root .env for auto data (autos_detalles_diarios)
load_dotenv()
SUPABASE_AUTOS_URL = os.environ.get("SUPABASE_URL")
SUPABASE_AUTOS_KEY = os.environ.get("SUPABASE_KEY")

# whatsapp_bot/.env for CRM data (crm_leads)
dotenv_path_crm = os.path.join(os.path.dirname(__file__), '..', 'whatsapp_bot', '.env')
load_dotenv(dotenv_path_crm, override=True)
SUPABASE_CRM_URL = os.environ.get("SUPABASE_URL")
SUPABASE_CRM_KEY = os.environ.get("SUPABASE_KEY")
...
def get_supabase_client():
    """Get Supabase client for AUTO data"""
...
def get_crm_supabase_client():
    """Get Supabase client for CRM data"""
    if not SUPABASE_CRM_URL or not SUPABASE_CRM_KEY:
        print("❌ Error: Missing SUPABASE_URL or SUPABASE_KEY for CRM.")
        sys.exit(1)
    return create_client(SUPABASE_CRM_URL, SUPABASE_CRM_KEY)
...
def analyze_price(target_data, market_df, url=None):
    ...
    # Send WhatsApp notification with contact info from CRM
    if url:
        # Try to get contact info from CRM (using CRM database)
        contact_info = ""
        try:
            supabase_crm = get_crm_supabase_client()
            crm_response = supabase_crm.table("crm_leads").select("name, phone").eq("car_url", url).execute()
            if crm_response.data:
                lead = crm_response.data[0]
                contact_name = lead.get('name', 'N/A')
                contact_phone = lead.get('phone', 'N/A')
